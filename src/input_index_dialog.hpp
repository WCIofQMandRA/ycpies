//input_index_dialog.hpp: 输入控制器程序索引的对话框
//包含类InputIndexDialog
...
//输入索引对话框
class InputIndexDialog: public wxDialog
{
public:
	InputIndexDialog(wxWindow *parent,const wxString &caption,const wxString &message,unsigned begin_index,unsigned end_index,
		const std::vector<unsigned> &program_list,unsigned default_index=10000);
	unsigned index()const;
private:
	bool contains(unsigned index)const;
	unsigned find_first_unused_index()const;
	const std::vector<unsigned> &program_list;
	unsigned begin_index,end_index,current_index;
	wxTextCtrl *m_edit;
